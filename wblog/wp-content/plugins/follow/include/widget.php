...
class S2_Form_widget extends WP_Widget {
	/**
	Declares the Subscribe widget class.
	*/
	function S2_Form_widget() {
		$widget_ops = array('classname' => 's2_form_widget', 'description' => __('Sidebar Widget for Follow Plugin', 'subscribe2') );
		$control_ops = array('width' => 250, 'height' => 300);
		$this->WP_Widget('s2_form_widget', __('Subscribe Widget', 'subscribe'), $widget_ops, $control_ops);
	}
	...
	*/
	function widget($args, $instance) {
		extract($args);
		$title = empty($instance['title']) ? __('Subscribe', 'subscribe2') : $instance['title'];
	...
	*/
	function form($instance) {
		...
		if ( $options === false ) {
			$defaults = array('title' => 'Subscribe', 'div' => 'search', 'widgetprecontent' => '', 'widgetpostcontent' => '', 'hidebutton' => 'none', 'postto' => '');
		} else {
		...
		if ( !empty($pages) ) {
			...
			echo "<option value=\"\">" . __('Use Default', 'subscribe2') . "</option>\r\n";
			global $mysubscribe2;
